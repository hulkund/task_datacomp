...
class GradMatch(EarlyTrain):
    def __init__(self, dst_train, args, fraction=0.5, random_seed=None, epochs=200, specific_model=None,
                 balance=True, dst_val=None, lam: float = 1., acf_mapping=None, **kwargs):
        ...
        with torch.no_grad():
            ...
            for i in range(budget):
                ...
                if indices.__len__() == 1:
                    ...
                    # try:
                    #     x_i = torch.linalg.lstsq(temp, rhs).solution
                    # except RuntimeError as e:
                    #     if "cusolver" in str(e).lower():
                    #         temp_cpu = temp.cpu()
                    #         rhs_cpu = rhs.cpu()
                    #         x_i_cpu = torch.linalg.lstsq(temp_cpu, rhs_cpu).solution
                    #         x_i = x_i_cpu.to(temp.device)
                    #     else:
                    #         raise
        ...
        with self.model.embedding_recorder:
            ...
            if self.balance:
                ...
                for c in range(self.args.num_classes):
                    ...
                    print(f"budget for class {c}: {budget}")
