...
def image_filter_helper(
    ...
    while True:
        ...
        if arch is not None:
            ...
            df = np.load(f"{path_root}", columns=["uid", "text", key], filesystem=fs)
            df_index = df[key] >= threshold
            df = df[df_index]
        else:
            df = np.load(f"{path_root}", columns=["uid", "text"], filesystem=fs)
...
# added
def load_uids_helper(filepath: str) -> np.ndarray:
    ...
    Returns:
        np.ndarray: array of uids
    """
    df = np.load(filepath, columns=["uid"], filesystem=fs)
